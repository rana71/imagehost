    ...
    <div class="panel-body">
        <div class='preview-container'>
            {{if $arrArtifact.description}}
                <blockquote>{{this::html($arrArtifact.description)}}</blockquote>
            {{end if-list}}
            {{BEGIN arrStoryElements}}
                <figure>
                    {{IF $type == 'image'}}
                    ...
                    {{ELSEIF $type == 'movie'}}
                        <iframe width="718" height="470" src="https://www.youtube.com/embed/{{$yt_id}}" frameborder="0" allowfullscreen></iframe>
                    {{END if-list}}
