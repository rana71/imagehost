...
class ManageArtifacts extends \webcitron\Subframe\Box {
    ...
    public function launch () {
        ...
        if (!empty($arrArtifacts)) {
            $arrArtifactsIds = array_column($arrArtifacts, 'id');

            $objArtifactModel = new ArtifactModel();
            $arrElements = $objArtifactModel->getElements($arrArtifactsIds);

            foreach ($arrArtifacts as & $arrArtifact) {
                $numTotalShows += intval($arrArtifact['shows_count_real']);
                $arrArtifact['boolOnlyImages'] = true;
                foreach ($arrElements as $arrElement) {
                    if ($arrElement['item_id'] === $arrArtifact['id'] && $arrElement['type'] !== ArtifactModel::ITEM_TYPE_IMAGE) {
                        $arrArtifact['boolOnlyImages'] = false;
                        break;
                    }
                }
            }
